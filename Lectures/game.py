# Lecture from 27/2/24 creating a game using classes
...
class Health(Item):
    def apply_effect(self, other):
        ...
        print("Player has recieved health")
...
class Room:
    item = [Health, HeartBox]
    enemies = [Zombie, Goblin]

    def __init__(self):
        self.__obj = choice(self.item+self.enemies)()
        ...
        if isinstance(room_obj, Character):
            while True:
                ...
                if room_obj.current_health <= 0:
                    print(f"The player is victorius")
                    return player
